import { ServerPlayerList } from '@interfaces/archeage.interface';
import { Player, History } from '@interfaces/player.interface';

import { FRACTION_NAME_CODES, PLAYER_STATUS } from '@configs/archeage';

export function compareCharacters(prev: Player[], next: Player[]): [Player[], Player[], Player[], History[]] {
  ...
  for (const updatedPlayer of next) {
    ...
    if (!player) {
      ...
      if (updatedPlayer.guild) {
        changes.guild_status = PLAYER_STATUS.Join;
      };
      newPlayers.push(updatedPlayer);
      history.push(changes);
    } else if (updatedPlayer.guild !== player.guild || updatedPlayer.fraction !== player.fraction) {
  ...
  for (const player of prev) {
    ...
    if (!updatedPlayer && player.num < 500) {
      const changes: History = {
        name: player.name,
        guild: player.guild,
        num: player.num,
...
export function splitCandidatesByFractions(players: Player[]): [Player[], Player[], Player[]];
export function splitCandidatesByFractions(players: ServerPlayerList): [Player[], Player[], Player[]];
export function splitCandidatesByFractions(players: Player[] | ServerPlayerList): [Player[], Player[], Player[]] {
  let piratePlayers: Player[], westPlayers: Player[], eastPlayers: Player[];
  if (Array.isArray(players)) {
    piratePlayers = players.filter(player => player.fraction === FRACTION_NAME_CODES.Pirates);
    westPlayers = players.filter(player => player.fraction === FRACTION_NAME_CODES.West);
    eastPlayers = players.filter(player => player.fraction === FRACTION_NAME_CODES.East);
  } else {
